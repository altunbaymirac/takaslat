...
export default function ListingQASection({ listingId, ownerId }: Props) {
  const { qas, askQuestion, answerQuestion, deleteQA, currentUser, currentUserId } = useAppStore();

  const myQAs = useMemo(
    () => qas.filter((q) => q.listingId === listingId).sort((a, b) => b.createdAt.localeCompare(a.createdAt)),
    [qas, listingId]
  );
  ...
  function handleAsk(e: FormEvent) {
    ...
    if (!question.trim() || question.trim().length < 5) {
      showToast('Soru en az 5 karakter olmalı', 'error');
      return;
    }
    askQuestion(listingId, question.trim());
    setQuestion('');
    showToast('Sorun yayınlandı', 'success');
  }

  function handleAnswer(qaId: string, e: FormEvent) {
    e.preventDefault();
    if (!answerText.trim()) return;
    answerQuestion(qaId, answerText.trim());
    setAnswerText('');
    setAnswerTo(null);
                ...
                <div className="ml-10">
                  {answerTo === qa.id ? (
                    <form onSubmit={(e) => handleAnswer(qa.id, e)} className="flex gap-2">
                      ...
                      />
                      <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-3 py-2 rounded-lg">Yanıtla</button>
