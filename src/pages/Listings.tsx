          ...
          <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto py-1">
            {recentlyViewedListings.slice(0, 8).map(l => (
              <Link
                key={l.id}
                to={`/listing/${l.id}`}
                className="flex w-56 flex-shrink-0 overflow-hidden rounded-md border border-slate-200 bg-white transition-colors hover:border-slate-300 dark:border-slate-700 dark:bg-slate-800"
              >
                <img src={l.images[0]} alt="" className="h-14 w-16 shrink-0 object-cover" />
                <div className="min-w-0 p-2">
                  <p className="text-xs font-semibold text-slate-800 dark:text-slate-100 line-clamp-1">{l.title}</p>
                  <p className="mt-0.5 text-xs font-bold text-blue-600 dark:text-blue-400">{formatPrice(l.estimatedValue)}</p>
                </div>
              </Link>
            ))}
          </div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
                >
