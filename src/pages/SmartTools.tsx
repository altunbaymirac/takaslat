      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[420px_1fr]">
        <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            ...
            <button disabled={aiLoading} onClick={() => runAI('Pazar Analizi', () => aiMarketInsights() as unknown as Promise<Record<string, unknown>>)} className="rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Pazar</button>
            <button disabled={!aiText.trim() || aiLoading} onClick={() => runAI('AI Takas Senaryoları', () => aiScenarios({ sourceListingId: sourceId || undefined, targetText: aiText, maxCashDiff: Number(cashDiff) || undefined }) as unknown as Promise<Record<string, unknown>>)} className="rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">Senaryo</button>
          </div>
